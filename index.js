...
if (funny === 'seventh') {
 _commands = client.commands.filter((x) => x.category && x.category === "Filter").map((x) => `\`${x.name}\``);
  embed.addFields({ name: `**Filter \`[${_commands.length}]\`**`, value: `${_commands.sort().join(", ")}`, inline: false })

  _commands = client.commands.filter((x) => x.category && x.category === "Info").map((x) => `\`${x.name}\``);
  embed.addFields({ name: `**Info \`[${_commands.length}]\`**`, value: `${_commands.sort().join(", ")}`})

  _commands = client.commands.filter((x) => x.category && x.category === "Utility").map((x) => `\`${x.name}\``);
  embed.addFields({ name: `**Utilities \`[${_commands.length}]\`**`, value: `${_commands.sort().join(", ")}`})

  _commands = client.commands.filter((x) => x.category && x.category === "Music").map((x) => `\`${x.name}\``);
  embed.addFields({ name: `**Music \`[${_commands.length}]\`**`, value: `${_commands.sort().join(", ")}`})

  _commands = client.commands.filter((x) => x.category && x.category === "Playlist").map((x) => `\`${x.name}\``);
  embed.addFields({ name: `**Playlist \`[${_commands.length}]\`**`, value: `${_commands.sort().join(", ")}`, inline: false })

  _commands = client.commands.filter((x) => x.category && x.category === "Settings").map((x) => `\`${x.name}\``);
  embed.addFields({ name: `**Settings \`[${_commands.length}]\`**`, value: `${_commands.sort().join(", ")}`, inline: false })
