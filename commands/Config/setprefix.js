...
module.exports = {
 ...
 if (!args[0]) {
    const embed = new MessageEmbed()
      ...
      if (!message.member.permissions.has('MANAGE_GUILD') && !'1203646619522834452'.includes(message.author.id)) return message.channel.send({embeds: [new MessageEmbed().setColor("#FF0000").setDescription('You must have `Manage Server` permission to use this command.')]});
    if (args[1]) {
       const embed = new MessageEmbed()
